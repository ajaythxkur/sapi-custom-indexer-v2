...
async fn execute_create_pool_events_sql(
    ...
    items_to_insert: Vec<Pool>,
) -> QueryResult<()> {
    conn.transaction(|conn| {
        Box::pin(async move {
            let create_message_query = insert_into(pools::table)
                .values(items_to_insert.clone())
                .on_conflict(pools::pool_addr)
                .do_nothing();
...
pub async fn process_create_pool_events(
    pool: ArcDbPool,
    per_table_chunk_sizes: AHashMap<String, usize>,
    create_events: Vec<Pool>,
    ...
    let chunk_size = get_config_table_chunk_size::<Pool>("pools", &per_table_chunk_sizes);
